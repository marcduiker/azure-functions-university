...
|6|[Change the route for a custom greeting](#6-change-the-route-for-a-custom-greeting)
...
4. Select the `.NET 6` (isolated) as the runtime.
    ...
    > 📝 **Tip** - More information about the isolated process can be found in the [official Azure documentation](https://docs.microsoft.com/en-us/azure/azure-functions/dotnet-isolated-process-guide).
...
8. Select `Function` for the AccessRights.
    ...
    |local.settings.json|Contains [app settings and connectionstrings](https://docs.microsoft.com/en-us/azure/azure-functions/functions-run-local?tabs=v4%2Cwindows%2Ccsharp%2Cportal%2Cbash%2Ckeda#local-settings) for local development.
...
2. The `req` parameter is of type `HttpRequestData`. Although this type has [various properties and methods](https://docs.microsoft.com/en-us/dotnet/api/microsoft.azure.functions.worker.http.httprequestdata?view=azure-dotnet) it does not contain anything to read query string values from the url. To read query string values, we can use the `HttpUtility.ParseQueryString()` method. The `HttpUtility` class lives in the `System.Web` namespace, so add this to the using directives at the top of the file:
...
## 6. Change the route for a custom greeting
