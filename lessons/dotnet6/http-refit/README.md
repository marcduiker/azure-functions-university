...
|1|[Cleaning up the source code](#1-cleaning-up-the-source-code)
...
This lesson builds on the [HTTP Trigger (.NET 6)](../http/README.md) lesson. Please, make sure to complete that lesson first.

## 1. Cleaning up the source code

First, remove the `HelloWorldHttpTrigger5.cs`, `HelloWorldHttpTrigger6.cs` and `Person.cs` files and only leave the `HelloWorldHttpTrigger.cs` source code file as well as the source code files generated by the project template.

This is the state you reached when completing step 4 of the previous lesson.
