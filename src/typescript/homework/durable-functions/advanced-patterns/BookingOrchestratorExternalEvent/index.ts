...
const orchestrator = df.orchestrator(function* (context) {
    ...
    const checkResult = yield context.df.callActivity("CheckHotelRoomApprovalNeededActivity", context.bindingData.input)
    ...
    if (checkResult === "approved" || orderApproved === true) {
        const orderResult = yield context.df.callActivity("BookHotelRoomActivity", context.bindingData.input)

        return orderResult
    }
    else {
        const message = "Booking of hotel room was declined"
